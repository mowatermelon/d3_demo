var draw = function() {
    ...
    // 平移变量
    var panSpeed = 200;
    var panBoundary = 20; // 拖动时，距离边缘20像素以内会平移.
    // 其他. 变量
    var i = 0;
    var duration = 750;
    var root;
    ...
    // Define the drag listeners for drag/drop behaviour of nodes.定义节点的拖放行为的拖放监听器。
    dragListener = d3.behavior.drag()
        .on("dragstart", function(d) {
            ...
            // it's important that we suppress the mouseover event on the node being dragged. Otherwise it will absorb the mouseover event and the underlying node will not detect it d3.select(this).attr('pointer-events', 'none');我们必须抑制正在拖动的节点上的鼠标悬停事件。 否则，它将吸收mouseover事件，并且底层节点不会检测到它。d3.select（this）.attr（'pointer-events'，'none'）;
        })
        .on("drag", function(d) {
            if (d == root) {
                return;
            ...
            // get coords of mouseEvent relative to svg container to allow for panning获取mouseEvent的相对于svg容器的坐标以允许平移
            ...
            domNode = this;
            if (selectedNode) {
                // now remove the element from the parent, and insert it into the new elements children现在从父项中移除该元素，并将其插入到新元素子项中
                ...
                // Make sure that the node being added to is expanded so user can see added node is correctly moved确保要添加的节点已扩展，以便用户可以看到添加的节点已正确移动
                expand(selectedNode);
                sortTree();
                endDrag();
            } else {
    ...
    function endDrag() {
        ...
        // now restore the mouseover event or we won't be able to drag a 2nd time现在恢复鼠标悬停事件，否则我们将无法第二次拖动
    ...
    // Function to update the temporary connector indicating dragging affiliation用于更新临时连接器的功能，指示从属关系
    var updateTempConnector = function() {
        var data = [];
        if (draggingNode !== null && selectedNode !== null) {
            // have to flip the source coordinates since we did this for the existing connectors on the original tree必须翻转源坐标，因为我们对原始树上的现有连接器进行了这样的操作
            data = [{
                source: {
                    x: selectedNode.y0,
                    y: selectedNode.x0
    ...
    function update(source) {
        ...
        // phantom node to give us mouseover in a radius around it幻像节点给我们鼠标在它周围的半径
        nodeEnter.append("circle")
            .attr('class', 'ghostCircle')
            .attr("r", 30)
            .attr("opacity", 0.2) // change this to zero to hide the target area将其更改为零以隐藏目标区域
        .style("fill", "red")
        ...
        // Change the circle fill depending on whether it has children and is collapsed根据是否有子节点并折叠，更改圆圈填充
